...
## プロンプトエンジニアリング/RAG/Fine Tuningの選択
  誤ったアプローチを選択すると、期待した成果が得られないばかりか、時間やリソースを無駄にする。<br>例えば、頻繁に更新される事実情報をファインチューニングで教え込もうとするのは非効率的で、RAGの方が適している。<br>逆に、特定の複雑な出力フォーマットを一貫して生成させたい場合は、プロンプトエンジニアリングだけでは限界があり、ファインチューニングが有効な手段となり得る。
...
1. 量より質を優先する（特にトレーニング初期）
    * 多数の低品質な例よりも、少数の非常によく練られた例から始めるのが効果的。まずは、質の高い50~100個の例で試し、効果を見極める。
2. 現実性を追及する
    * トレーニング例は、実際のアプリケーションでモデルが遭遇すると想定されるプロンプトと、それに対する理想的な出力を可能な限り忠実に反映させる。現実離れした例や実際のユースケースと乖離した例では、モデルは実用的な能力を学習できない。
3. 明確かつ具体的
...
9. 既存の例を精査する
    * データセット内の例に文法的な誤り、論理的な矛盾、不適切なスタイルなどが含まれていないかを確認する。モデルはこれらの欠点も学習してしまう可能性がある。
...
* ハイパーパラメータ
  * batch_size, learning_rate_multiplier, n_epochs, seedなど
  * ※最近のOpenAI apiでは、多くのハイパーパラメータが自動的に最適化される傾向にあり、ユーザーが直接指定できる項目は限られる場合がある。
* suffix
